...
int main()
{
    ...
    printf("삽입 데이터 2개 입력 : ");
    scanf("%s %s", value, value1);
    printf("=======================================\n\n");

    /* send */
    snprintf(send_buffer, sizeof(send_buffer), "INSERT INTO USER_TB VALUE ('%s', '%s')", value, value1);
    send_packet(sock_info.fd, SQL_INSERT, send_buffer);
    sleep(1);
...
void type_categorizer(Packet packet, int fd){
	if(packet.header.type == SQL_INSERT){
        printf("=======================================\n");
        printf("\t   %s\n", packet.buf);
        printf("=======================================\n\n");
    }
